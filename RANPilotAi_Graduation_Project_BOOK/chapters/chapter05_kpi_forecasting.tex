\chapter{KPI Forecasting \& Predictive Analytics Module}
...
Fixed regularization constants are \texttt{colsample\_bytree}=0.8, \texttt{min\_child\_weight}=3, \texttt{reg\_alpha}=0.1, and \texttt{reg\_lambda}=1.0, chosen to combat overfitting given the limited temporal depth of the dataset. Figure~\ref{fig:hyperparameters} shows the Streamlit sidebar through which operators adjust these parameters interactively.

% --- FIGURE 5.2: Hyperparameters sidebar ---
\begin{figure}[H]
    \centering
...
This protocol ensures that the system does not overstate the value of machine learning when the underlying series is inherently unpredictable or stable. Figure~\ref{fig:metrics_panel} shows the UI presentation of this comparison for cell \texttt{LCAIN10022-6}.

% --- FIGURE 5.3: Metrics panel ---
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/chapter05/image3.png}
    \caption{Dashboard header for cell \texttt{LCAIN10022-6} forecasting DL Traffic Volume. The seasonality strength badge ($F_{\text{seasonal}}=0.87$) and baseline comparison panel confirm that Holt-Winters (MAE 6.352) substantially outperforms the naive baseline (MAE 16.384), validating model deployment for this cell.}
...
% --- FIGURE 5.5: Residual summary statistics ---
\begin{figure}[H]
    \centering
...
% --- FIGURE 5.6: Four-panel residual diagnostics ---
\begin{figure}[H]
    \centering
